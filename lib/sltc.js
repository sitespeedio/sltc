...
const packageLossScripts = [
  'sudo tc qdisc change dev %s root netem loss %s'
]


let deleteScripts = ['sudo tc qdisc del dev %s root netem'];
    ...
    sltc: function(config) {
        ...
        if (options.remove) {
            ...
            delayScripts.forEach((script) => {
                execSync(util.format(script, options.device, options.delay));
            })
